    ...
    char *vhost_name)                   /*  Virtual host used for request    */
{
    ...
    _size = put_xixlog_log (&_body, ip_client, ip_server, user_name, service, request, query, method, response, size_recd, size_sent, time_ms, referer, agent, from, file_name, vhost_name);
    if (_size)
      {
